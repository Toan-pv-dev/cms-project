        ...
        <div class="component-filter d-flex ">
                ...
                <select name="perpage" class="form-control w-100">
                    @for ($i = 20; $i <= 200; $i += 20)
                        <option {{ $perpage == $i ? 'selected' : '' }} value="{{ $i }}">
                            {{ $i }}
                            {{ __('messages.productCatalogue.record') }}</option>
                    @endfor
                </select>
            </div>
            @php
            ...
            <div class="perpage-select col-lg-3">
                <select name="publish" class="form-control publish">
                    @if (is_array(config('apps.general.publish')))
                        ...
                        @endforeach
                    @else
                        <option value="-1">{{ __('messages.productCatalogue.available_option') }} </option>
                    @endif
            ...
            <div class="search-input-form col-lg-4 col-md-6 mb-2 mr5">
                ...
                <button type="submit" class="btn btn-info w-100"
                    style="background-color: #1ab394;">{{ __('messages.productCatalogue.search') }} </button>
            </div>

            <div class="filter-add-component ">
                <a href="{{ route('product.catalogue.create') }}"><button type="button" class="btn btn-warning w-100">
                        <i style="margin-right: 6px"
                            class="glyphicon glyphicon-plus"></i>{{ __('messages.productCatalogue.create_index') }}</button></a>
            </div>
        </div>
    </div>
</form>
