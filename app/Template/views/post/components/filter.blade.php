<form action="{{ route('moduleName.index') }}">
    <div class="filter-wrapper">
        ...
        <div class="component-filter d-flex ">
            @php
                ...
                $moduleNameCatalogueId = request('moduleName_catalogue_id') ?: old('moduleName_catalogue_id');
            ...
            <div class="right-component-filter col-lg-6 col-md-3 mb-2 mr-10 p0">
                <div class=" perpage-select col-lg-3 col-md-4">
                    <select name="moduleName_catalogue_id" class="form-control setupSelect2 publish">
                        @foreach ($dropdown as $key => $val)
                            <option {{ $moduleNameCatalogueId == $key ? 'selected' : '' }} value="{{ $key }}">
                                {{ $val }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class=" filter-add-component  ">
                    <a href="{{ route('moduleName.create') }}"><button type="button" class="btn btn-warning w-100">
                            <i style="margin-right: 6px"
                                class="glyphicon glyphicon-plus"></i>{{ config('apps.moduleName.create_index') }}</button></a>
                </div>
            </div>
        </div>
    </div>
